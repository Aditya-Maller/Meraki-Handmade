...
def get_inquiries():
    conn = get_db_connection()
    inquiries = conn.execute('SELECT * FROM contact').fetchall()
    conn.close()
    inquiries_list = [dict(ix) for ix in inquiries]
...
def send_confirmation():
    if request.method == 'POST':
        email = request.form['email']
        items = request.form.getlist('items')  # Assuming items are submitted as a list
        ...
        return redirect(url_for('send_confirmation', invoice_number=invoice_number))
